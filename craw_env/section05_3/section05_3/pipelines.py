...
class NewsPipeline(object):
    ...
    def __init__(self):
        # DB 설정(자동 커밋)
        self.conn = sqlite3.connect('E:/py_workout/database_db.db', isolation_level=None)
        # DB 연결
        self.c = self.conn.cursor()

    # 최초 1회 실행
    def open_spider(self, spider):
        spider.logger.info('News Pipline Started.')
    ...
    def process_item(self, item, spider):
        if not item.get('contents') is None:
            ...
            # 데이터 -> DB 삽입
            self.c.execute('INSERT INTO NEWS_DATA(headline, contents, parent_link, article_link, crawled_time) VALUES (?, ?, ? ,? ,?);', (item.get('headline'), item.get('contents'), item.get('parent_link'), item.get('article_link'), item.get('crawled_time')))
            # tuple(item[k] for k in item.keys())
    ...
    def close_spider(self, spider):
        spider.logger.info('News Pipline Stoped.')
        # 커밋
        self.conn.commit()
        # 연결 해제
        self.conn.close()
